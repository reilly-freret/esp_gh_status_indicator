...
static esp_err_t parse_json_field(const char *json, const char *field_name, char *value, size_t value_size)
{
...
static esp_err_t get_deployment_id(const char *environment, char *deployment_id, size_t id_size)
{
    ...
    esp_http_client_set_header(client, "Authorization", "token " CONFIG_GITHUB_AUTH_TOKEN);
...
static esp_err_t get_deployment_status(const char *deployment_id, char *status, size_t status_size)
{
