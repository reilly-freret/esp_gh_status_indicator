...
esp_err_t utils_init(void)
{
    ...
    ESP_LOGI(TAG, "SNTP initialized with server pool.ntp.org, waiting for time synchronization...");
...
esp_err_t get_human_real_time(char *timestamp)
{
    ...
    if (hour_12 == 0) hour_12 = 12; // Convert 0 to 12 for 12-hour format
    
    snprintf(timestamp, 16, "%02d:%02d:%02d", 
             hour_12, 
             timeinfo.tm_min, 
             timeinfo.tm_sec);
